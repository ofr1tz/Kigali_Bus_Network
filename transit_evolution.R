...
bbox <- c(29.95, -2.10, 30.25, -1.80)

map <- get_map(location=bbox, source="stamen", color="bw", scale="auto")
...
osmTimeSeries <- function(date) {
    ...
    g + geom_sf(data=routes, size=1, color="red", inherit.aes=FALSE) +
        ...
        geom_text(x=bbox[3]-.002, y=bbox[2]+.002, hjust=1, vjust=0, 
                  label="Map tiles by Stamen Design, under CC BY 3.0. Data by OpenStreetMap, under ODbL.",
                  fontface="plain", size=3.5, colour="grey60") +
    ...
    ggsave(filename=paste0("./output/OSM_Kigali_Transit_", date,".png"),
           device="png", width = 10, height=10, dpi = 150)
}

seq(as.Date("2018/02/23"), Sys.Date(), "week") %>% 
    map_df(osmTimeSeries)

list.files(path = "./output/", pattern = "*.png", full.names = T) %>% 
    map(image_read) %>% 
    image_join() %>% 
    image_animate(fps=4) %>% 
